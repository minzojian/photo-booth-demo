...
export default function Releases() {
  ...
  const columns: ProColumns<ReleaseRow>[] = [
    ...
    {
      title: intl.formatMessage({ id: 'releases.size' }),
      dataIndex: 'size',
      width: 90,
      render: (_, r) => `${(r.size / 1024 / 1024).toFixed(1)} MB`,
    },
    {
    ...
    },
    {
      ...
      dataIndex: 'enabled',
      width: 80,
      render: (_, r) => <Switch checked={r.enabled} onChange={() => handleToggle(r)} />,
    },
    {
        ...
        <Form form={form} layout="vertical" style={{ marginTop: 16 }} disabled={uploading}>
          ...
          <div style={{ marginBottom: 24 }}>
            <div style={{ marginBottom: 8, color: 'rgba(0,0,0,.88)', fontSize: 14 }}>
              {intl.formatMessage({ id: 'releases.upload.zip' })} <span style={{ color: '#ff4d4f' }}>*</span>
            </div>
            <Upload
              accept=".zip"
              maxCount={1}
              ...
              <Button icon={<UploadOutlined />} disabled={uploading}>
                {intl.formatMessage({ id: 'releases.upload.zip' })}
              </Button>
            </Upload>
          </div>
          <div style={{ marginBottom: 24 }}>
            <div style={{ marginBottom: 8, color: 'rgba(0,0,0,.88)', fontSize: 14 }}>
              {intl.formatMessage({ id: 'releases.upload.blockmap' })}
            </div>
            <Upload
              accept=".blockmap"
              maxCount={1}
          ...
          <Form.Item
            name="releaseNotes"
            label={intl.formatMessage({ id: 'releases.releaseNotes' })}
            rules={[{ required: true, message: '请填写更新日志' }]}>
          ...
          {uploading && (
            <div style={{ color: '#1677ff', fontSize: 13 }}>
              {intl.formatMessage({ id: 'releases.uploading' })}
              {pctZip > 0 && ` ${pctZip}%`}
