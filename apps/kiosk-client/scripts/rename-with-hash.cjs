...
/** 获取 app-builder 原生二进制路径（与 electron-builder 内部逻辑完全一致） */
function getAppBuilderPath() {
  try {
    // app-builder-bin 自身导出了正确的平台/架构路径
    return require('app-builder-bin').appBuilderPath
  } catch {
    console.warn('[rename-with-hash] app-builder-bin not available, blockmap skipped')
    return null
  }
}
...
function generateBlockmapNative(zipPath, blockmapPath) {
  ...
  const binaryPath = getAppBuilderPath()
  if (!binaryPath) return
  if (!existsSync(binaryPath)) {
    console.warn('[rename-with-hash] app-builder binary not found:', binaryPath)
    return
  }
  const result = spawnSync(binaryPath, ['blockmap', '--input', zipPath, '--output', blockmapPath], {
    stdio: 'pipe',
    encoding: 'utf8',
  })
  if (result.status !== 0) {
...
function renameZip(zipPath, options = {}) {
  ...
  if (!m) {
    console.log('[rename-with-hash] skip (cannot parse version):', base)
    return
  }
  ...
  if (base === newBase) {
    console.log('[rename-with-hash] skip (already renamed):', base)
    return
  }

  if (pruneSameVersion) {
    cleanupSameVersionArtifacts(outDir, m.groups.version, m.groups.archOs, [
      base,
      `${base}.blockmap`,
      newBase,
      newBlockmapBase,
  ...
  try {
    renameSync(oldBlockmap, newBlockmap)
    console.log('[rename-with-hash] blockmap renamed')
  } catch {
    /* no blockmap */
  }
  ...
  if (!existsSync(newBlockmap)) {
    try {
      generateBlockmapNative(newZipPath, newBlockmap)
    } catch (e) {
      console.warn('[rename-with-hash] blockmap generation failed:', e.message)
    }
  }
...
if (require.main === module) {
  ...
  exports.default = async function renameWithHash(buildResult) {
    const artifactPaths = (buildResult.artifactPaths || []).filter(
      (p) => (p.endsWith('.zip') || p.endsWith('.exe')) && !p.endsWith('.zip.blockmap') && !p.endsWith('.exe.blockmap'),
    )
