import type { FrameTheme } from "./boothConfig";
import { CAPTURE_SIZE } from "./boothConfig";
...
/** 把模板背景图 + 所有已拍照片按 frames 定义合成到一张 Canvas，用于编辑阶段的完整预览 */
export async function composeWithTemplate(
  shots: Shot[],
  template: { bgUrl: string; bgW: number; bgH: number; frames: { x: number; y: number; width: number; height: number }[] }
): Promise<HTMLCanvasElement> {
  const canvas = document.createElement("canvas");
  canvas.width = template.bgW;
  canvas.height = template.bgH;
  const ctx = canvas.getContext("2d")!;

  // 先绘制照片，再覆盖模板图层，确保相框元素位于最上层。
  for (let i = 0; i < shots.length && i < template.frames.length; i++) {
    const shot = shots[i];
    const frame = template.frames[i];
    const img = await loadImage(shot.dataUrl);

    const fx = frame.x * template.bgW;
    const fy = frame.y * template.bgH;
    const fw = frame.width * template.bgW;
    const fh = frame.height * template.bgH;

    drawImageCover(ctx, img, fx, fy, fw, fh);
  }

  const bgImg = await loadImage(template.bgUrl);
  ctx.drawImage(bgImg, 0, 0, template.bgW, template.bgH);

  // 贴纸绘制在最上层（坐标 0~1 相对画布）
  for (const shot of shots) {
    for (const s of shot.stickers) {
      const fontPx = Math.max(template.bgW * 0.05, 28);
      ctx.font = `${fontPx}px "Apple Color Emoji","Segoe UI Emoji",sans-serif`;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(s.emoji, s.x * template.bgW, s.y * template.bgH);
    }
  }
